...
const createNewMatrix = () => {
  ...
  matrix.forEach(() => {
    matrix[Math.floor(Math.random() * 9) + 1][
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const audioBoom = new Audio(
    "./sounds/zvuki_iz_igry_saper_minesweeper_xMh.mp3"
  );
      ...
      if (i != 0 && i != 10) {
        for (let j = 1; j < 10; j++) {
          ...
          if (item[j] != 0) {
            ...
            if (item[j] === 1)
              cell.style.textShadow = "rgb(0 22 255) 1px 0 10px";
            if (item[j] === 2)
              cell.style.textShadow = "rgb(77 255 0) 1px 0 10px";
            if (item[j] === 3)
              cell.style.textShadow = "rgb(217, 0, 255) 1px 0 10px";
          }
          if (item[j] < 0)
            cell.innerHTML =
  ...
  const checkWinning = (point, bomb) => {
    if (point === bomb) {
      ...
      modal.innerHTML = `<div><img src="./img/map_13899955.png"></div>
            <div class="winner">Congratulations!<br> time : ${addZero(
              time.hours
  ...
  const formQ = (queue) => {
    while (queue.length != 0) {
      if (
        ...
        const hidingCell = document.querySelector(
          `#c${queue[0][0]}${queue[0][1]}`
        );
      ...
      if (e.target.id) {
        ...
        if (Number(underCell.innerHTML) > 0 && underCell.innerHTML != "") {
          ...
          modal.innerHTML = `<div><img src="./img/explosion_5512962.png"></div>
                    <div class="looser">You have lost! <br> time : ${addZero(
                      time.hours
                    )}
                    :${addZero(time.minutes)}:${addZero(time.seconds)}
  ...
  score.addEventListener("click", () => {
    ...
    keys.forEach((item) => {
      const timeData = getTime(
        JSON.parse(myStorage.getItem(`${item}`)).duration
      );
      const element = document.createElement("div");
      element.classList.add("game_info");
      element.innerHTML = `<p><span class="color"> ${gameNumber}. </span>
            <span>game duration: <span class="color">${addZero(
              timeData.hours
            )}:${addZero(timeData.minutes)}:${addZero(
        timeData.seconds
      )}</span> </span>
            <span>steps: <span class="color">${
              JSON.parse(myStorage.getItem(`${item}`)).steps
            }</span></span>
            <span>outcome of the game: <span class="color">${
              JSON.parse(myStorage.getItem(`${item}`)).victory
                ? "victory"
                : "defeat"
            }</span></span></p>`;
      gameNumber++;
